...
public enum ErrorCode {
    ...
    /** 资源不存在（id/业务键均未命中；W2 prd 域 CRUD 注册） */
    PLT_4040("T1-PLT-4040", 404, "资源不存在"),
    /** 授权链未通过（四步短路链默认拒绝） */
    ...
    /** 发布存在未完结工作项（R-9 发布一致性硬门禁，4230 同族 422；明细按类型分组列计数与 key 样例） */
    PRD_4231("T1-PRD-4231", 422, "发布存在未完结工作项"),
    ...
    /** 消息已超出可撤回时间窗（M2-INC-2：teamone.im.withdraw-window-hours，默认 24h） */
    COL_4209("T1-COL-4209", 403, "消息已超出可撤回时间窗"),
    ...
    /** 分支受保护，未满足保护策略（如最少批准数、单测门禁） */
    ENG_4253("T1-ENG-4253", 422, "未满足分支保护合并策略"),
    /** 基线已定版冻结，为不可变对象 */
    ENG_4254("T1-ENG-4254", 422, "基线已冻结，禁止修改或覆盖"),
    /** 违反分支管理策略（branch_rule：建分支名不匹配任何 name_pattern / MR 目标不符规则 merge_target） */
    ENG_4255("T1-ENG-4255", 422, "违反分支管理策略"),
    ...
    /** 依赖不可用（Valkey/MinIO/Git 仓库目录 超时或失联，可重试） */
